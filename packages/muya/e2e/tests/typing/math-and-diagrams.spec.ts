...
test.describe('math and diagrams', () => {
    ...
    test('clicking the rendered inline KaTeX activates the source for editing', async ({ page }) => {
        ...
        // The preview wrapper is the hit target; KaTeX is painted inside it
        // and Playwright correctly reports the wrapper intercepting the child.
        await page.locator(`${editor.inlineMath} ${editor.mathRender}`).first().click();
