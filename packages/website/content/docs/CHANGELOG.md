...
Inkiva's document-first release. The editor keeps standard Markdown files and folders at the center of the workflow.

**Features**

- Recent documents and folders on the welcome surface, with pin, remove, clear, and reveal actions.
- Quick Open, document tabs, optional two-pane comparison, outline navigation, and folder-wide Markdown search.
- Standard Markdown links, backlinks, image paths, tables, diagrams, and code blocks remain portable.
- Local history, autosave/recovery protection, updater validation, and release artifact checks are part of the quality gate.
- Official desktop packages are published for Windows x64 and macOS Intel/Apple Silicon.

**Compatibility**

- Existing CommonMark/GFM documents remain plain Markdown; Inkiva does not add Wiki Links, hidden markers, or a workspace database.
- macOS packages are currently unsigned/not notarized; Gatekeeper may require an explicit first launch approval.

## v0.2.0

- Established the Inkiva brand, three application appearances, and a neutral Paper surface.
- Added reliable session restore, autosave queueing, updater state handling, performance checks, and the initial Windows/macOS packaging contract.

## v0.1.0

- Initial Inkiva desktop release with WYSIWYG Markdown editing, source mode, folder browsing, export, themes, and standard Markdown compatibility.
...
Inkiva is now Inkiva! We'd like to thank all contributors and users that have been using Inkiva and worked on this release.
...
- Inkiva is now single-instance application on Linux and Windows too.
...
- Inkiva can use a default directory that is automatically opened during startup (#711)
...
- Support maxOS `dark mode`, when you change `mode dark or light` in system, Inkiva will change its theme.
...
- Restore default Inkiva style after exporting/printing
...
- Fixed list parse error [more info](https://github.com/Bulls1986/Inkiva/issues/831#issuecomment-477719256)
- Fixed source code mode tab switching
- Fixed source code mode to preview switching
- Inkiva didn't remove highlight when I delete the markdown symbol like * or `. (#893)
...
- feat: #487 Show filename while hovering over inkiva file on dock
...
You need uninstall the old version of Inkiva before install version 0.10.21, because we changed the AppId when build.
...
- Add user preferences in `Inkiva menu`, the shoutcut is `CmdorCtrl + ,`, you can set the default `theme` and `autoSave`.
- Add `autoSave` to `file menu`, the default value is in `preferences.md` which you can open in `Inkiva menu`. #45
- Add drag and drop to open Markdown file with Inkiva @fxha
...
- fix: prevent open image or file directly when drag and drop over Inkiva #42
