...
const cli = (): ParsedArgs => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to Inkiva and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'inkiva-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: inkiva [commands] [path ...]
...
`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'inkiva-user-data')
