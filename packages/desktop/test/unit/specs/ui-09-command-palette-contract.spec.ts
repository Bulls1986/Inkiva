...
describe('Inkiva UI-09 command launcher contract', () => {
  it('uses a responsive launcher surface with explicit keyboard semantics', () => {
    ...
    expect(palette).toContain('!isBusy && !availableCommands.length')
    expect(palette).toContain("t('commandPalette.sections.headings')")
    expect(palette).toContain('headingSearchResults')
