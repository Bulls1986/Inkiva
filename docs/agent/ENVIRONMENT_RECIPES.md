...
- Playwright payload vs launcher: a missing `.bin/playwright` or failed `pnpm exec playwright` does **not** mean Playwright is absent. Check `node_modules/playwright/cli.js` and `node_modules/@playwright/test` first. On 2026-09-24 a worktree had the full Playwright 1.61.0 payload but no usable package-local launcher; invoking the real JS CLI with the trusted Node executable discovered and executed the intended current-worktree specs.
- Root Junction health: when a worktree root `node_modules` is itself a Junction, inspect its target before diagnosing individual missing tools. A verified failure mode is a Junction pointing at an incomplete main-checkout graph: top-level Playwright/electron-vite packages exist, but transitive payloads such as `cac`, `picocolors`, Electron, or package-local launchers are incomplete. Before repointing the **root-level** Junction to a warm donor, require matching `pnpm-lock.yaml`, package-manager/engines, root dependencies/devDependencies, and affected package manifests; script-only root-package differences do not invalidate dependency reuse. Fingerprint equality is necessary but not sufficient: probe representative dependency realpaths (`require.resolve` plus native realpath) for the affected subsystem and reject a donor whose internal links escape into a different worktree/version graph. A 2026-09-24 Muya type build exposed exactly this failure as mixed `marked@18` / `marked@16` types.
- Electron build readiness: `electron-vite --version` proves only that the CLI boots. A real current-worktree build must also resolve `electron/package.json` and produce current `out/main`, `out/preload`, and `out/renderer`. If the matching root donor lacks Electron at the physical path required by electron-vite/E2E, add only a version-matching `electron` Junction (root and/or `packages/desktop/node_modules/electron` as required); never Junction the whole desktop dependency tree.
- Long Windows build/test launch: when the active NVM path contains spaces, bypass the NVM/.cmd wrapper by invoking the real trusted `node.exe` with the JS CLI. For renderer builds that legitimately exceed the short shell budget, use one long-lived process Job and observe that same Job to terminal state. Never reuse another branch's `out/`. In the 2026-09-24 recovery, the current-worktree Electron build completed main + preload + renderer after the dependency topology was repaired, and focused US09 Playwright completed `3/3` with normal worker teardown.
...
If the silent push never reaches `git-remote-https` / credential-helper / `send-pack` and even Trace2 fails to create its event file, inspect the Runner process table before changing transport settings. A verified failure mode is stale double-layer `git.exe` processes (for example completed `rev-parse HEAD` or earlier `push` invocations) left with zero CPU long after the caller returned. Confirm they are stale by command line, start time, CPU and absence of an active logical Job, then terminate only those proven stale Git processes. Do not kill unrelated Git activity from another worktree/session. If a clean retry still stalls at the same pre-transport point, stop experimenting with push variants and use the controlled GitHub Git Data API fallback below.
...
| `.bin/playwright` missing but `node_modules/playwright/cli.js` exists | run the real JS CLI with the trusted Node executable and current-worktree config/source; require real test discovery | declare Playwright absent / install random launchers |
| root `node_modules` Junction points at an incomplete donor | fingerprint lockfile + package-manager/engines + dependency manifests, then repoint only the root Junction to a matching healthy donor | keep patching missing transitive packages one-by-one |
| donor fingerprint matches but representative dependency realpaths escape to another worktree/version | reject/repair that donor and re-probe realpaths before product validation | trust lockfile equality alone / cast away mixed-type errors |
| electron-vite boots but cannot resolve `electron/package.json` | add only a version-matching Electron physical/Junction path required by the build/helper | Junction the whole `packages/desktop/node_modules` tree |
| current-worktree renderer build is healthy but exceeds short shell budget | run one real-Node build process with a long bounded timeout and observe the same Job | reuse donor `out/` / launch duplicate builds |
