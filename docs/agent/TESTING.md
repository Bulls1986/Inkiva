...
- Treat fixed sleeps as debt, not as a default readiness mechanism. Prefer observable application state, DOM conditions, IPC/events, or bounded polling. For keyboard/input E2E, the driver promise only proves event dispatch; wait for the exact public editor state/Markdown contract before asserting when rendering/state propagation can lag under parallel load. Do not delete a sleep until the replacement condition proves the same contract, especially for debounce/history/negative-condition timing windows.
